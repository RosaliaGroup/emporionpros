...
const EP = {
  ...
  ai: {
    ...
    respond: async function(userMsg) {
      ...
      // ── Property Search ──────────────────────────────────
      if (msg.match(/find|search|look|show|list|propert|home|house|apartment|condo|rent|buy/)) {
        ...
        const forRent = msg.includes('rent') || msg.includes('apartment');
        ...
        if (filtered.length) {
          const list = filtered.map(p => `• **${p.title}** — $${p.price.toLocaleString()}${p.type==='rent'?'/mo':''} | ${p.beds} bed | ${p.sqft.toLocaleString()} sqft | ${p.address}`).join('\n');
      ...
      if (msg.match(/price|cost|market|worth|value|afford/)) {
        return "Newark median sale price is currently **$435,000** — up 8.2% year over year. Rentals average **$2,400/mo** for 2-beds. Would you like a free property valuation for a specific address?";
      }
