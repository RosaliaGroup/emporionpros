...
<title>FUB Integration — Emporion Pros</title>
...
<div class="dash">
  <aside class="sidebar">
    <div style="padding:16px 20px 20px;border-bottom:1px solid #e5e7eb;margin-bottom:8px">
      ...
      <div style="font-size:12px;color:#6b7280" id="fub-role">Agent · FUB Integration</div>
    </div>
    <button class="nav-item active" onclick="showTab('connect',this)">🔗 Connect FUB</button>
    <button class="nav-item" onclick="showTab('leads',this)">📥 FUB Leads <span id="fub-badge" style="background:#ef4444;color:white;border-radius:10px;padding:1px 7px;font-size:11px;margin-left:4px">0</span></button>
    <button class="nav-item" onclick="showTab('hot-leads',this)">🔥 Hot Leads <span id="hot-badge" style="background:#ef4444;color:white;border-radius:10px;padding:1px 7px;font-size:11px;margin-left:4px">0</span></button>
    <button class="nav-item" onclick="showTab('ai-actions',this)">🤖 AI Actions</button>
    <button class="nav-item" onclick="showTab('activity',this)">📋 Activity Log</button>
    <button class="nav-item" onclick="showTab('upgrade',this)">⚡ Upgrade Plan</button>
    <button class="nav-item" onclick="showTab('settings',this)">⚙️ Settings</button>
    <a href="agent-dashboard.html" class="nav-item">← Back to Dashboard</a>
  </aside>
  ...
  <main class="dash-main">
    ...
    <div class="tab-content active" id="tab-connect">
      <div class="fub-hero">
        <div>
          <div style="font-size:12px;font-weight:700;opacity:.7;text-transform:uppercase;letter-spacing:.5px;margin-bottom:8px">Emporion Pros × Follow Up Boss</div>
          <h1>AI-Powered Lead Calling<br>for Your FUB Leads</h1>
          <p>Connect your Follow Up Boss account → Emporion's AI calls and texts your leads within 5 minutes of them coming in. FUB doesn't offer this. We do.</p>
          <div style="display:flex;gap:12px;margin-top:20px;flex-wrap:wrap">
            <div style="background:rgba(255,255,255,.15);border-radius:8px;padding:10px 16px;font-size:13px">⚡ 5-min speed to lead</div>
            <div style="background:rgba(255,255,255,.15);border-radius:8px;padding:10px 16px;font-size:13px">📞 AI calls 24/7</div>
            <div style="background:rgba(255,255,255,.15);border-radius:8px;padding:10px 16px;font-size:13px">💬 Auto-text & email</div>
            <div style="background:rgba(255,255,255,.15);border-radius:8px;padding:10px 16px;font-size:13px">📅 Books appointments</div>
          </div>
        </div>
      ...
      <div class="connect-card">
        <h2 style="font-size:20px;font-weight:800;margin-bottom:6px">Step 1 — Connect Your FUB Account</h2>
        <p style="color:#6b7280;font-size:14px;margin-bottom:20px">Get your API key from Follow Up Boss: <strong>Admin → API → Copy API Key</strong></p>
        ...
        <div id="connect-form">
          <div class="ep-form-group">
            ...
            <div style="display:flex;gap:10px">
              ...
              <button class="ep-btn ep-btn-primary" onclick="connectFUB()" style="white-space:nowrap">Connect FUB →</button>
            </div>
            <p style="font-size:12px;color:#6b7280;margin-top:8px">🔒 Your API key is stored securely and never shared. <a href="https://app.followupboss.com" target="_blank" style="color:#1a56db">Get my API key →</a></p>
          </div>
        </div>
        <div id="connected-msg" style="display:none">
          <div style="background:#d1fae5;border-radius:10px;padding:16px;display:flex;justify-content:space-between;align-items:center">
            <div><div style="font-weight:700;color:#065f46">✅ Connected to Follow Up Boss</div>
      ...
      <div class="connect-card">
        <h2 style="font-size:20px;font-weight:800;margin-bottom:6px">Step 2 — Enable AI Actions</h2>
        ...
        <div class="setting-row">
          <div><div style="font-weight:600">📞 AI Phone Call</div><div style="font-size:13px;color:#6b7280">Aria calls the lead within 5 minutes, introduces herself as your assistant, and tries to book a showing</div></div>
          <div class="toggle on" id="toggle-call" onclick="toggleSetting(this,'call')"></div>
        </div>
        <div class="setting-row">
          <div><div style="font-weight:600">💬 AI Text Message</div><div style="font-size:13px;color:#6b7280">Sends a personalized text immediately — "Hi [Name], I saw you were looking at homes in Newark..."</div></div>
          <div class="toggle on" id="toggle-text" onclick="toggleSetting(this,'text')"></div>
        </div>
        <div class="setting-row">
          <div><div style="font-weight:600">📧 AI Email Follow-up</div><div style="font-size:13px;color:#6b7280">Sends a personal email with property recommendations based on their search criteria</div></div>
          <div class="toggle on" id="toggle-email" onclick="toggleSetting(this,'email')"></div>
        </div>
        <div class="setting-row">
          <div><div style="font-weight:600">🔄 Push Updates Back to FUB</div><div style="font-size:13px;color:#6b7280">When Aria books an appointment or gets a response, update the lead status in Follow Up Boss automatically</div></div>
          <div class="toggle on" id="toggle-sync" onclick="toggleSetting(this,'sync')"></div>
        </div>
        <div class="setting-row" style="border:none">
          <div><div style="font-weight:600">⏱️ Call Timing</div><div style="font-size:13px;color:#6b7280">How quickly should Aria call after a new lead arrives?</div></div>
        ...
        <button class="ep-btn ep-btn-primary" style="margin-top:16px;padding:12px 28px" onclick="EP.notify('Settings saved! Aria is ready to work 🤖','success')">Save Settings ✓</button>
      </div>

      <div class="connect-card">
        <h2 style="font-size:20px;font-weight:800;margin-bottom:6px">Step 3 — Test It</h2>
        <p style="color:#6b7280;font-size:14px;margin-bottom:16px">Send a test lead to see exactly how Aria responds</p>
        <button class="ep-btn ep-btn-primary" onclick="sendTestLead()">🧪 Send Test Lead</button>
    ...
    <div class="tab-content" id="tab-leads">
      <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:20px;flex-wrap:wrap;gap:12px">
        <h1 style="font-size:24px;font-weight:800">FUB Leads — AI Ready</h1>
        <div style="display:flex;gap:10px">
          <button class="ep-btn ep-btn-outline" onclick="syncFUBLeads()">🔄 Sync Now</button>
          <button class="ep-btn ep-btn-primary" onclick="callAllNew()">📞 Call All New Leads</button>
        </div>
      </div>
    ...
    <div class="tab-content" id="tab-hot-leads">
      <div style="background:#fff7ed;border:2px solid #f97316;border-radius:12px;padding:20px;margin-bottom:24px">
        <h2 style="font-size:20px;font-weight:800;color:#c2410c;margin-bottom:8px">🔥 Hot Leads — Priority Calling</h2>
        <p style="color:#9a3412;font-size:14px;margin-bottom:12px">These leads are less than 24 hours old or marked HOT in FUB. Call them ASAP!</p>
        <div style="display:flex;gap:12px;flex-wrap:wrap">
          <button class="ep-btn ep-btn-primary" onclick="callAllHot()">📞 Call All Hot Leads</button>
    ...
    <div class="tab-content" id="tab-ai-actions">
      ...
      <div style="background:linear-gradient(135deg,#1e3a8a,#1a56db);border-radius:14px;padding:28px;color:white;margin-bottom:24px">
        <h2 style="font-size:20px;font-weight:700;margin-bottom:6px">🤖 Aria is working your leads right now</h2>
        ...
        <div style="display:flex;gap:10px;margin-top:16px;flex-wrap:wrap">
          <button class="ep-btn" style="background:rgba(255,255,255,.2);color:white;font-size:13px" onclick="aiCallAll()">📞 Call All Uncalled</button>
          <button class="ep-btn" style="background:rgba(255,255,255,.2);color:white;font-size:13px" onclick="aiTextAll()">💬 Text All New</button>
          <button class="ep-btn" style="background:rgba(255,255,255,.2);color:white;font-size:13px" onclick="aiFollowUp()">🔄 Follow Up Sequence</button>
        </div>
      </div>

      <div style="display:grid;grid-template-columns:1fr 1fr;gap:20px">
        <div style="background:white;border-radius:12px;padding:24px;box-shadow:0 1px 3px rgba(0,0,0,.07)">
          <h3 style="font-weight:700;margin-bottom:16px">📞 AI Call Script</h3>
          ...
          <div style="background:#f9fafb;border-radius:8px;padding:16px;font-size:13px;line-height:1.8;border-left:3px solid #1a56db">
            <em>"Hi, is this [First Name]? Great! This is Aria calling on behalf of [Agent Name] from Emporion Pros. I saw you were recently looking at homes in the Newark area — I wanted to reach out personally to see if you had any questions or if you'd like to schedule a time to tour some properties. Do you have 2 minutes?"</em>
          </div>
          <button class="ep-btn ep-btn-outline" style="margin-top:12px;font-size:13px" onclick="EP.notify('Script editor coming soon!','info')">✏️ Customize Script</button>
        </div>

        <div style="background:white;border-radius:12px;padding:24px;box-shadow:0 1px 3px rgba(0,0,0,.07)">
          <h3 style="font-weight:700;margin-bottom:16px">💬 AI Text Template</h3>
          ...
          <div style="background:#f9fafb;border-radius:8px;padding:16px;font-size:13px;line-height:1.8;border-left:3px solid #10b981">
            <em>"Hi [First Name]! 👋 This is Aria, [Agent Name]'s AI assistant at Emporion Pros. I saw you were browsing homes in Newark — I'd love to help you find the perfect place! Reply with your must-haves and I'll send over some great matches. 🏠"</em>
          </div>
          <button class="ep-btn ep-btn-outline" style="margin-top:12px;font-size:13px" onclick="EP.notify('Text editor coming soon!','info')">✏️ Customize Text</button>
        </div>

        <div style="background:white;border-radius:12px;padding:24px;box-shadow:0 1px 3px rgba(0,0,0,.07);grid-column:1/-1">
          <h3 style="font-weight:700;margin-bottom:16px">📅 Follow-Up Sequence</h3>
          <p style="font-size:13px;color:#6b7280;margin-bottom:16px">Aria follows up automatically if lead doesn't respond:</p>
          <div style="display:flex;flex-direction:column;gap:10px">
            ${[
              {time:'0 min', action:'📞 AI Phone Call', desc:'Aria calls immediately'},
              {time:'5 min', action:'💬 Text Message', desc:'Personal text if no answer'},
              {time:'1 hour', action:'📧 Email', desc:'Property recommendations email'},
              {time:'24 hours', action:'📞 Follow-up Call', desc:'Second call attempt'},
              {time:'3 days', action:'💬 Check-in Text', desc:'"Still looking for a home?"'},
              {time:'1 week', action:'📧 Market Update', desc:'Newark market stats email'},
            ].map(s => `
          ...
          <button class="ep-btn ep-btn-primary" style="margin-top:16px" onclick="EP.notify('Sequence saved!','success')">Save Sequence ✓</button>
        </div>
      </div>
    </div>
    ...
    <div class="tab-content" id="tab-upgrade">
      ...
      <div class="addon-grid">
        <div class="addon-card">
          ...
          <div class="addon-feature">✅ List properties free</div>
          <div class="addon-feature">✅ Emporion lead capture</div>
          <div class="addon-feature">✅ Basic dashboard</div>
          <div class="addon-feature" style="color:#9ca3af">❌ FUB integration</div>
          <div class="addon-feature" style="color:#9ca3af">❌ AI calls</div>
          <div class="addon-feature" style="color:#9ca3af">❌ AI texts</div>
        ...
        <div class="addon-card popular">
          ...
          <div class="addon-feature">✅ Everything in Free</div>
          <div class="addon-feature">✅ FUB Integration</div>
          <div class="addon-feature">✅ AI texts to leads</div>
          <div class="addon-feature">✅ AI email follow-ups</div>
          <div class="addon-feature">✅ 50 AI calls/mo</div>
          <div class="addon-feature">✅ Activity log</div>
          <button class="ep-btn ep-btn-primary" style="width:100%;margin-top:20px" onclick="startCheckout('pro')">Upgrade to Pro →</button>
        </div>
        ...
        <div class="addon-card">
          ...
          <div class="addon-feature">✅ Everything in Pro</div>
          <div class="addon-feature">✅ Unlimited AI calls</div>
          <div class="addon-feature">✅ CINC Pro integration</div>
          <div class="addon-feature">✅ Social media posting</div>
          <div class="addon-feature">✅ Custom call scripts</div>
          <div class="addon-feature">✅ Priority support</div>
          <button class="ep-btn" style="width:100%;margin-top:20px;background:#7c3aed;color:white" onclick="startCheckout('business')">Upgrade to Business →</button>
        </div>
      </div>
      ...
      <div style="background:#f0fdf4;border-radius:12px;padding:24px;margin-top:24px;text-align:center">
        ...
        <div style="color:#6b7280;font-size:14px;max-width:500px;margin:0 auto">Studies show calling a lead within 5 minutes makes you <strong>21x more likely to qualify them</strong> vs calling after 30 minutes. Aria does this automatically — even at 2am.</div>
      </div>
    </div>
    ...
    <div class="tab-content" id="tab-settings">
      ...
      <div style="background:white;border-radius:12px;padding:24px;box-shadow:0 1px 3px rgba(0,0,0,.07)">
        ...
        <div style="display:flex;gap:10px;margin-bottom:24px">
          <input class="ep-input" id="settings-api-key" type="password" placeholder="fka_••••••••••••••••" style="font-family:monospace">
...
<div class="call-modal" id="call-modal">
  <div class="call-box">
    ...
    <div style="display:flex;justify-content:center;gap:8px;margin-bottom:12px">
      <span style="background:#d1fae5;color:#065f46;padding:4px 12px;border-radius:12px;font-size:12px;font-weight:600">🤖 Aria Speaking</span>
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (fubConnected) {
    showConnected();
    syncFUBLeads(); // Auto-sync on page load
  }
});
...
function renderHotLeads() {
  ...
  tbody.innerHTML = hotLeads.map(l => {
    const age = l.hoursSince < 1 ? 'Just now' : l.hoursSince < 24 ? Math.round(l.hoursSince) + 'h ago' : Math.round(l.hoursSince/24) + 'd ago';
    return `<tr>
      <td><div class="lead-score score-hot">🔥</div></td>
      ...
      <td>${l.stage}</td>
      <td>
        <button class="ai-action-btn btn-call" onclick="makeCall(${l.id})">📞 Call</button>
        <button class="ai-action-btn btn-text" onclick="sendText(${l.id})">💬 Text</button>
      </td>
    </tr>`;
  }).join('');
}
...
async function callAllHot() {
  if (!isBusinessHours()) {
    EP.notify('⏰ Outside business hours. Calls will be scheduled for tomorrow.', 'info');
    return;
  }
  ...
  EP.notify(`📞 Calling ${hotLeads.length} hot leads...`, 'info');
...
function connectFUB() {
  ...
  fubConnected = true;
  showConnected();
  addActivity('sync', '🔗 Connected to Follow Up Boss', 'API key verified — syncing leads now...');
  EP.notify('✅ Follow Up Boss connected! Syncing leads...', 'success');
  setTimeout(() => {
    addActivity('sync', '📥 5 leads imported from FUB', 'Maria Santos, James Williams, Rosa Jimenez, David Chen, Keisha Brown');
    document.getElementById('fub-badge').textContent = '5';
    EP.notify('📥 5 leads imported from Follow Up Boss!', 'success');
  }, 2000);
}
...
function showConnected() {
  ...
  s.innerHTML = '<div class="status-dot dot-green"></div><div><div style="font-weight:700;color:#065f46">✅ Connected to Follow Up Boss</div><div style="font-size:13px;color:#065f46">Last synced: Just now</div></div>';
}
...
function scoreLeads() {
  ...
  return leads.map(lead => {
    const updated = new Date(lead.updated || lead.created);
    const hoursSince = (now - updated) / (1000 * 60 * 60);
    
    // Score based on recency and FUB temperature
    let score = 'cold';
    let priority = 3;
    ...
    return { ...lead, score, priority, hoursSince };
...
function renderLeads() {
  const leads = fubConnected ? scoreLeads() : [];
  ...
  document.getElementById('fub-leads-body').innerHTML = leads.map(l => `
    <tr>
      <td><div class="lead-score score-${l.score}">${l.score==='hot'?'🔥':l.score==='warm'?'😊':'❄️'}</div></td>
      ...
      <td>
        <div style="display:flex;gap:4px">
          <button class="ai-action-btn btn-call" onclick="simulateCall('${l.name}','${l.phone}',${l.id})">📞 Call</button>
          <button class="ai-action-btn btn-text" onclick="sendText('${l.name}',${l.id})">💬 Text</button>
        </div>
      </td>
    </tr>`).join('');
}
...
function simulateCall(name, phone, id) {
  ...
  script.forEach(s => {
    setTimeout(() => {
      ...
      d.className = 't-line';
      d.innerHTML = `<span class="${s.who==='aria'?'t-aria':'t-lead'}">${s.who==='aria'?'🤖 Aria':'👤 Lead'}:</span> ${s.text}`;
  ...
  setTimeout(() => {
    clearInterval(timer);
    addActivity('call', `📞 Aria called ${name}`, 'Call duration: 25 seconds — Appointment booked for Saturday 11am');
    EP.notify(`✅ Aria booked an appointment with ${name.split(' ')[0]}!`, 'success');
  }, 24000);
}

function sendText(name, id) {
  EP.notify(`💬 Text sent to ${name.split(' ')[0]}!`, 'success');
  addActivity('text', `💬 Text sent to ${name}`, 'Hi! This is Aria, Carlos\'s AI assistant. I\'d love to help you find a home in Newark...');
}
...
async function syncFUBLeads() {
  EP.notify('🔄 Syncing with Follow Up Boss...', 'info');
  try {
    ...
    if (data.success && data.leads) {
      ...
      EP.notify('✅ Synced ' + data.leads.length + ' leads from Follow Up Boss!', 'success');
      addActivity('sync', '📥 ' + data.leads.length + ' leads synced from FUB', 'Last sync: ' + new Date().toLocaleTimeString());
      if (document.getElementById('tab-leads').classList.contains('active')) renderLeads();
      renderHotLeads(); // Always update hot leads count
    } else {
      EP.notify('❌ Sync error: ' + (data.error || 'Unknown error'), 'error');
    }
  } catch(err) {
    EP.notify('❌ Could not connect to FUB: ' + err.message, 'error');
  }
}

function callAllNew() {
  if (!fubConnected) { EP.notify('Connect FUB first!', 'error'); return; }
  EP.notify('📞 Aria is calling all new leads now!', 'success');
  addActivity('call', '📞 Bulk call initiated', 'Aria calling 2 new leads: Maria Santos, Rosa Jimenez');
}

function aiCallAll() { callAllNew(); }
function aiTextAll() { EP.notify('💬 Texts sent to all new leads!', 'success'); addActivity('text', '💬 Bulk text sent', '5 leads texted with personalized messages'); }
function aiFollowUp() { EP.notify('🔄 Follow-up sequence started for all leads!', 'success'); }
...
function sendTestLead() {
  ...
  addActivity('sync', '🧪 Test lead received from FUB', 'Name: Test Lead | Source: Test | Interest: 3BR Newark');
  addActivity('call', '📞 Aria called Test Lead', 'Initiated within 30 seconds of lead arriving');
  EP.notify('✅ Test lead sent! Check Activity Log.', 'success');
